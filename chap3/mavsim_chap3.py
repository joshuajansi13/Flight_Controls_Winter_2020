...
while sim_time < SIM.end_time:
    ...
    if sim_time < SIM.end_time / 12:
        ...
        fz = 150
        forces_moments = np.array([[fx, fy, fz, Mx, My, Mz]]).T
    elif sim_time < 7 * SIM.end_time / 12:
        fz = 0
        Mx = 0.1
        forces_moments = np.array([[fx, fy, fz, Mx, My, Mz]]).T
    elif sim_time < 8 * SIM.end_time / 12:
        Mx = -0.1
        forces_moments = np.array([[fx, fy, fz, Mx, My, Mz]]).T
    elif sim_time < 9 * SIM.end_time / 12:
        Mx = 0
        My = 0.1
        forces_moments = np.array([[fx, fy, fz, Mx, My, Mz]]).T
    elif sim_time < 10 * SIM.end_time / 12:
        My = -0.1
        forces_moments = np.array([[fx, fy, fz, Mx, My, Mz]]).T
    elif sim_time < 11 * SIM.end_time / 12:
        My = 0
        Mz = 0.1
        forces_moments = np.array([[fx, fy, fz, Mx, My, Mz]]).T
    elif sim_time < 12 * SIM.end_time / 12:
        Mz = -0.1
        forces_moments = np.array([[fx, fy, fz, Mx, My, Mz]]).T
    else:
        fx = 10
